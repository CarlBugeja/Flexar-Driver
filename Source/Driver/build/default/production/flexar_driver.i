...
void trimmer_handler(void){
    ...
    adc_avgsum = adc_avgsum >> 3U;


    index_val = adc_avgsum >> 4U;

    if(index_val != index_lastval){

        uint16_t array_val = index_val << 4U;
        if(index_tmr < 250U){
            index_tmr++;
        }
        else{
            index_lastval = index_val;
                if(driver_mode <= 2U){
                    ...
                    period_val = period_buffer[index_val];
                    period_halfval = period_val >> 1U;
                }
...
void driver_init(void){
    ...
    driver_mode = DATAEE_ReadByte(0x7000U);
    ...
    PWM5_LoadDutyValue(125U);
    PWM6_LoadDutyValue(125U);
...
void buzzer_pwm_config(void){
    ...
    T2CON = 0x07;
    PR2 = 128U;
    PWM5_LoadDutyValue(258U);
    PWM6_LoadDutyValue(258U);
